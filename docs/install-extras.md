...
| `sight` | `agentvision[render,motion,desktop]` | The **eyes** — visual gating of rendered UIs, plus **local motion media** (video files + animated GIFs) graded **over time**, the temporal `watch`, and **live desktop screen capture** (`xdg-desktop-portal`). |
