...
def test_bandit_spec_gates_medium_and_excludes_nonshipped():
    """The security grader gates on MEDIUM+ severity (real SQLi/crypto/injection) and must not drown in
    test-only patterns (every `assert` is B101), vendored code under .venv, or non-shipped dev tools —
    else it can never pass on a normal project (dogfooding finding)."""
    from verel.ci.graders import bandit_spec

    cmd = bandit_spec("/repo").command
    assert "--severity-level" in cmd and cmd[cmd.index("--severity-level") + 1] == "medium"
    assert "--confidence-level" in cmd
    exclude = cmd[cmd.index("--exclude") + 1]
    assert all(d in exclude for d in ("./tests", "./.venv", "./tools"))
