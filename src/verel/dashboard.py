...
Run:  verel-dashboard                              # loopback http://127.0.0.1:8042 (zero-config)
      VEREL_DASHBOARD_HOST=0.0.0.0 VEREL_DASHBOARD_TOKEN=… \
        VEREL_DASHBOARD_CERT=cert.pem VEREL_DASHBOARD_KEY=key.pem verel-dashboard   # routable: needs auth+TLS
...
def _get(url: str, timeout: float = 15.0, retries: int = 2) -> dict | None:
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=timeout) as r:  # nosec B310 — scheme guarded http(s) above
